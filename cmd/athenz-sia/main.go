...
func parseFlags(program string, args []string) (*identity.IdentityConfig, error) {
	...
	f.StringVar(&backup, "backup", backup, "backup certificate to Kubernetes secret (\"\", \"read\", \"write\", \"read+write\" or \"file\" must be run uniquely for each secret to prevent conflict)")
