...
var logger *logrus.Entry = log.WithField("component", "certificate")
...
func New(ctx context.Context, idCfg *config.IdentityConfig) (daemon.Daemon, error) {
	if ctx.Err() != nil {
		logger.Info("Skipped certificate provider initiation")
		return nil, nil
	}

	// TODO: This log should be moved to derived-service-cert.go
	if !idCfg.ServiceCert.CopperArgos.Use {
		logger.Infof("Certificate provisioning is disabled with empty options: provider service[%s]", idCfg.ServiceCert.CopperArgos.Provider)
	}

	// TODO: This log should be moved to derived-role-cert.go
	if !idCfg.RoleCert.Use {
		logger.Infof("Role certificate provisioning is disabled with empty options: roles[%s], filename format[%s]", idCfg.RoleCert.TargetDomainRoles, idCfg.RoleCert.Format)
	}

	handler, err := InitIdentityHandler(idCfg)
	if err != nil {
		logger.Errorf("Failed to initialize client for certificates: %s", err.Error())
		return nil, err
	}
	...
	writeFiles := func() error {
		...
		if identity != nil && localFileKeyPEM == nil && localFileIdentity == nil {
			...
			if len(leafPEM) != 0 && len(keyPEM) != 0 {
				...
				logger.Infof("[New Instance Certificate] Subject: %s, Issuer: %s, NotBefore: %s, NotAfter: %s, SerialNumber: %s, DNSNames: %s",
				...
				for _, certFile := range idCfg.ServiceCert.CopperArgos.Cert.Paths {
					...
					logger.Debugf("Saving x509 cert[%d bytes] at %s", len(leafPEM), certFile)
				...
				for _, keyFile := range idCfg.ServiceCert.CopperArgos.Key.Paths {
					...
					logger.Debugf("Saving x509 key[%d bytes] at %s", len(keyPEM), keyFile)
			...
			if len(caCertPEM) != 0 && idCfg.CaCertFile != "" {
				logger.Debugf("Saving x509 cacert[%d bytes] at %s", len(caCertPEM), idCfg.CaCertFile)
		...
		if roleCerts != nil {
			for _, rolecert := range roleCerts {
				...
				if len(roleCertPEM) != 0 {
					logger.Infof("[New Role Certificate] Subject: %s, Issuer: %s, NotBefore: %s, NotAfter: %s, SerialNumber: %s, DNSNames: %s",
					...
					logger.Debugf("Saving x509 role cert[%d bytes] at [%s]", len(roleCertPEM), outPath)
					...
					if idCfg.RoleCert.KeyFormat != "" {
						...
						logger.Debugf("Saving x509 role cert key[%d bytes] at [%s]", len(roleKeyPEM), outKeyPath)
	...
	identityProvisioningRequest := func(forceInit bool) (err error, identity *InstanceIdentity, keyPEM []byte) {
		logger.Infof("Mapped Athenz domain[%s], service[%s]", handler.Domain(), handler.Service())

		identity, keyPEM, err = handler.GetX509Cert(forceInit)

		if err != nil {
			logger.Warnf("Error while requesting x509 certificate to identity provider: %s", err.Error())

			if idCfg.K8sSecretBackup.UseWrite {
				logger.Errorf("Failed to receive x509 certificate to update kubernetes secret[%s]: %s", idCfg.K8sSecretBackup.Secret, err.Error())
				return
			}

		} else {
			logger.Info("Successfully received x509 certificate from identity provider")

			if idCfg.K8sSecretBackup.UseWrite {

				logger.Infof("Attempting to save x509 certificate to kubernetes secret[%s]...", idCfg.K8sSecretBackup.Secret)

				err = handler.ApplyX509CertToSecret(identity, keyPEM)
				if err != nil {
					logger.Errorf("Failed to save x509 certificate to kubernetes secret: %s", err.Error())
					return
				}

				logger.Infof("Successfully saved x509 certificate to kubernetes secret")
			} else {
				logger.Debugf("Skipping to save x509 certificate temporary backup to Kubernetes secret[%s]", idCfg.K8sSecretBackup.Secret)
			}
		}
	...
	roleCertProvisioningRequest := func() (err error, roleCerts [](*RoleCertificate), roleKeyPEM []byte) {
		...
		logger.Infof("Attempting to get x509 role certs from identity provider: targets[%s]...", idCfg.RoleCert.TargetDomainRoles)

		roleCerts, roleKeyPEM, err = handler.GetX509RoleCert()
		if err != nil {
			logger.Warnf("Error while requesting x509 role certificate to identity provider: %s", err.Error())
			return err, nil, nil
		}

		logger.Info("Successfully received x509 role certs from identity provider")
		return nil, roleCerts, roleKeyPEM
	}

	run := func() error {
		if idCfg.ServiceCert.CopperArgos.Use {
			logger.Infof("Attempting to request x509 certificate to identity provider[%s]...", idCfg.ServiceCert.CopperArgos.Provider)

			err, identity, keyPEM = identityProvisioningRequest(false)
			if err != nil {
				logger.Errorf("Failed to retrieve x509 certificate from identity provider: %s", err.Error())
			}
			if identity != nil && len(keyPEM) != 0 {
				errUpdate := idCfg.Reloader.UpdateCertificate([]byte(identity.X509CertificatePEM), keyPEM)
				if errUpdate != nil {
					logger.Errorf("Failed to update x509 certificate into certificate reloader: %s", errUpdate.Error())
				}
			}
		} else if idCfg.ServiceCert.LocalCert.Use {
			logger.Debug("Attempting to load x509 certificate from cert reloader...")
			localFileKeyPEM, localFileCertPEM, err := idCfg.Reloader.GetLatestKeyAndCert()
			if err != nil {
				logger.Warnf("Error while reading x509 certificate key from cert reloader: %s", err.Error())
				return err
			}
			localFileIdentity, err = InstanceIdentityFromPEMBytes(localFileCertPEM)
			if err != nil {
				logger.Warnf("Error while parsing x509 certificate from cert reloader: %s", err.Error())
			}
			if localFileIdentity == nil || len(localFileKeyPEM) == 0 {
				logger.Errorf("Failed to load x509 certificate from cert reloader to get x509 role certs: key size[%d]bytes, certificate size[%d]bytes", len(localFileCertPEM), len(localFileKeyPEM))
			} else {
			...
			logger.Debugf("Skipping to request/load x509 certificate: identity provider[%s]", idCfg.ServiceCert.CopperArgos.Provider)
		}

		if identity == nil || len(keyPEM) == 0 {
			if idCfg.K8sSecretBackup.UseRead {
				logger.Infof("Attempting to load x509 certificate temporary backup from kubernetes secret[%s]...", idCfg.K8sSecretBackup.Secret)

				k8sSecretBackupIdentity, k8sSecretBackupKeyPEM, err = handler.GetX509CertFromSecret()
				if err != nil {
					logger.Warnf("Error while loading x509 certificate temporary backup from kubernetes secret: %s", err.Error())
				}

				if k8sSecretBackupIdentity == nil || len(k8sSecretBackupKeyPEM) == 0 {
					logger.Warnf("Failed to load x509 certificate temporary backup from kubernetes secret: secret was empty")
				} else {
					identity = k8sSecretBackupIdentity
					keyPEM = k8sSecretBackupKeyPEM
					logger.Infof("Successfully loaded x509 certificate from kubernetes secret")
					if identity != nil && len(keyPEM) != 0 {
						errUpdate := idCfg.Reloader.UpdateCertificate([]byte(identity.X509CertificatePEM), keyPEM)
						if errUpdate != nil {
							logger.Errorf("Failed to update x509 certificate into certificate reloader: %s", errUpdate.Error())
				...
				logger.Debugf("Skipping to load x509 certificate temporary backup from Kubernetes secret[%s]", idCfg.K8sSecretBackup.Secret)
			}
		}
		...
		if k8sSecretBackupIdentity != nil && len(k8sSecretBackupKeyPEM) != 0 && idCfg.ServiceCert.CopperArgos.Use {
			logger.Infof("Attempting to request renewed x509 certificate to identity provider[%s]...", idCfg.ServiceCert.CopperArgos.Provider)
			err, forceInitIdentity, forceInitKeyPEM = identityProvisioningRequest(true)
			if err != nil {
				logger.Warnf("Failed to retrieve renewed x509 certificate from identity provider: %s, continuing with the backup certificate from kubernetes secret", err.Error())
			} else {
		...
		if err != nil {
			if forceInitIdentity != nil || forceInitKeyPEM != nil {
				...
				logger.Errorf("Failed to save files for renewed key[%s], renewed cert[%s] and renewed certificates for roles[%v]", idCfg.ServiceCert.CopperArgos.Key.Raw, idCfg.ServiceCert.CopperArgos.Cert.Raw, idCfg.RoleCert.TargetDomainRoles)
			} else {
				logger.Errorf("Failed to save files for key[%s], cert[%s] and certificates for roles[%v]", idCfg.ServiceCert.CopperArgos.Key.Raw, idCfg.ServiceCert.CopperArgos.Cert.Raw, idCfg.RoleCert.TargetDomainRoles)
			}
		}
	...
	err = backoff.RetryNotify(run, ebo, func(err error, backoffDelay time.Duration) {
		logger.Errorf("Failed to get initial certificates: %s. Retrying in %s", err.Error(), backoffDelay)
	})
	if err != nil {
		// mode=init, must output preset certificates
		if idCfg.Init {
			logger.Errorf("Failed to get initial certificates after multiple retries for init mode: %s", err.Error())
			return nil, err
		}
		// mode=refresh, on retry error, ignore and continue server startup
		logger.Errorf("Failed to get initial certificates after multiple retries for refresh mode: %s, will try to continue startup process...", err.Error())
	}
...
func (cs *certService) Start(ctx context.Context) error {
	if ctx.Err() != nil {
		logger.Info("Skipped certificate provider start")
		return nil
	}
	...
	if cs.idCfg.Refresh > 0 {
		...
		go func() {
			...
			notifyOnErr := func(err error, backoffDelay time.Duration) {
				logger.Errorf("Failed to refresh certificates: %s. Retrying in %s", err.Error(), backoffDelay)
			}
			for {
				logger.Infof("Will refresh key[%s], cert[%s] and certificates for roles[%v] with provider[%s], backup[%s] and secret[%s] within %s",
				...
				case <-cs.shutdownChan:
					logger.Info("Stopped certificate provider daemon")
					return
				case <-t.C:
					// skip refresh if context is done but Shutdown() is not called
					if ctx.Err() != nil {
						logger.Infof("Skipped to refresh key[%v], cert[%v] and certificates for roles[%v] with provider[%s], backup[%s] and secret[%s]",
							cs.idCfg.ServiceCert.CopperArgos.Key.Raw, cs.idCfg.ServiceCert.CopperArgos.Cert.Raw,
					...
					if err != nil {
						logger.Errorf("Failed to refresh certificates after multiple retries: %s", err.Error())
					}
				}
			}
		}()
...
func (cs *certService) Shutdown() {
	logger.Info("Initiating shutdown of certificate provider daemon ...")
	...
	if cs.idCfg.DeleteInstanceID && cs.handler.InstanceID() != "" {
		logger.Info("Attempting to delete x509 certificate record from identity provider...")
		err := cs.handler.DeleteX509CertRecord()
		if err != nil {
			logger.Warnf("Failed to delete x509 certificate Instance ID record: %s", err.Error())
		} else {
			logger.Infof("Successfully deleted x509 certificate Instance ID record[%s]", cs.handler.InstanceID())
		}
	}
}
