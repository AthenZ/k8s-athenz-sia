...
func New(ctx context.Context, idCfg *config.IdentityConfig) (daemon.Daemon, error) {
	...
	identityProvisioningRequest := func(forceInit bool) (err error, identity *InstanceIdentity, keyPEM []byte) {
		...
		if err != nil {
			...
			if idCfg.K8s.Secret.BackupCert != "" && strings.Contains(idCfg.Backup, "write") {
				log.Errorf("Failed to receive x509 certificate to update kubernetes secret[%s]: %s", idCfg.K8s.Secret.BackupCert, err.Error())
				return
			}
		} else {
			...
			if idCfg.K8s.Secret.BackupCert != "" && strings.Contains(idCfg.Backup, "write") {
				...
				log.Infof("Attempting to save x509 certificate to kubernetes secret[%s]...", idCfg.K8s.Secret.BackupCert)
				...
				log.Debugf("Skipping to save x509 certificate temporary backup to Kubernetes secret[%s]", idCfg.K8s.Secret.BackupCert)
			}
		}
	...
	run := func() error {
		...
		if identity == nil || len(keyPEM) == 0 {
			if idCfg.K8s.Secret.BackupCert != "" && strings.Contains(idCfg.Backup, "read") {
				log.Infof("Attempting to load x509 certificate temporary backup from kubernetes secret[%s]...", idCfg.K8s.Secret.BackupCert)
				...
				log.Debugf("Skipping to load x509 certificate temporary backup from Kubernetes secret[%s]", idCfg.K8s.Secret.BackupCert)
			}
		}
...
func (cs *certService) Start(ctx context.Context) error {
	...
	if cs.idCfg.Refresh > 0 {
		...
		go func() {
			...
			for {
				log.Infof("Will refresh key[%s], cert[%s] and certificates for roles[%v] with provider[%s], backup[%s] and secret[%s] within %s", cs.idCfg.KeyFile, cs.idCfg.CertFile, cs.idCfg.RoleCert.TargetDomainRoles, cs.idCfg.ProviderService, cs.idCfg.Backup, cs.idCfg.K8s.Secret.BackupCert, cs.idCfg.Refresh)
				...
				case <-t.C:
					...
					if ctx.Err() != nil {
						log.Infof("Skipped to refresh key[%s], cert[%s] and certificates for roles[%v] with provider[%s], backup[%s] and secret[%s]", cs.idCfg.KeyFile, cs.idCfg.CertFile, cs.idCfg.RoleCert.TargetDomainRoles, cs.idCfg.ProviderService, cs.idCfg.Backup, cs.idCfg.K8s.Secret.BackupCert)
						continue
					}
