...
func New(ctx context.Context, idCfg *config.IdentityConfig) (daemon.Daemon, error) {
	...
	accessTokenCache := NewLockedTokenCache("accesstoken", idCfg.K8s.Ns, idCfg.K8s.Po.Name)
	roleTokenCache := NewLockedTokenCache("roletoken", idCfg.K8s.Ns, idCfg.K8s.Po.Name)
